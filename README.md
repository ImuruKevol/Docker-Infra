...
| 도메인/SSL | DDNS endpoint, certbot 무료 인증서, nginx server block 자동 생성/검증/rollback |
...
- DDNS key, AI Agent 세션/설정 secret, backup system secret은 암호화 저장하거나 runtime data directory에 두고 API 응답에서는 마스킹합니다.
...
| Domains/SSL | DDNS endpoints, certbot certificates, automatic nginx server block generation and rollback |
...
- DDNS keys, AI Agent session/config secrets, and backup system secrets are encrypted at rest or kept in the runtime data directory and masked in API responses.
