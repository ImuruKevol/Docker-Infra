...
The `installer/` directory is intended to be copied as a self-contained install unit. Its `payload/` directory carries the packaged WIZ bundle, Python requirements, and checksums so the production host does not need the development workspace. Payload checksums are verified before use.
...
The install flow also installs Node.js LTS and npm from NodeSource, then installs the official `@openai/codex` npm package globally as the normal `codex` command. API-token based OpenAI, Gemini, and Ollama execution uses provider HTTP APIs directly.
