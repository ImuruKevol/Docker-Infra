...
운영 초기 구성은 제품 첫 접속 화면이 아니라 installer HTML에서 다음만 받는다.

1. 관리자 비밀번호
2. 서비스 백업 시스템 구성 여부

installer의 `setup` 단계는 다음을 자동 처리한다.
...
초기 설치 이후 관리자 비밀번호는 시스템 설정의 General 탭에서 현재 비밀번호 검증 후 변경한다. 별도 사용자 계정 관리 화면은 두지 않고 `operator_auth`의 단일 관리자 credential을 유지한다.
