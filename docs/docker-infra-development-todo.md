...
서비스 백업 시스템은 기본 비활성화다. 활성화하면 마스터 노드에서 Harbor Compose를 자동 실행하고, 백업 저장소 용량과 상태를 Docker Infra가 관리한다.

필요한 TODO:

- [Done] 최초 구성 마법사에 `서비스 백업 시스템 구성` 토글 추가
- [Done] 토글 기본값 비활성화
- 활성화 시 마스터 노드에 Harbor Compose 설치/실행
...
- 실패 시 화면에서 복구 가능한 메시지와 재시도 버튼 제공
...
- 새 서비스 버튼은 wizard modal로 진입
- 단계 1: 서비스 이름과 설명
- 단계 2: 실행 이미지 선택
- 단계 3: 버전/tag 선택
- 단계 4: 내부 포트 자동 감지 또는 선택
- 단계 5: 공개 도메인 선택 또는 신규 도메인 연결
- 단계 6: 환경변수 폼 입력
- 단계 7: 볼륨/데이터 보관 폴더 선택
- 단계 8: 실행 서버 또는 자동 배치 선택
- 단계 9: 최종 요약과 배포 실행
- Compose YAML은 고급 모드에서만 표시
- 사용자가 직접 YAML을 수정한 경우에도 form 값과 충돌하는 부분을 명확히 표시

### 5.2 템플릿 기반 생성

필요한 TODO:

- 템플릿은 Docker Compose 원문보다 `필요 입력값 schema` 중심으로 관리
- 템플릿 선택 시 이름, 이미지, 포트, 환경변수, volume 필드를 자동 폼으로 매핑
- DB, WAS, reverse app, static app 같은 쉬운 분류 제공
...
- `docker stack deploy` 또는 `docker compose up -d` 기준을 명확히 결정
- 배포 로그는 Job이 아니라 operation output으로 표시
- 배포 성공 후 서비스 상태, 컨테이너 상태, health check 결과를 즉시 갱신
- 서비스 수정은 wizard form으로 제공
- Compose 원문 수정은 고급 모드
- rollback은 Compose 버전과 이미지 tag를 기준으로 단순화
- rollback 실행 전 영향 범위 확인 모달 제공
...
- nginx config 원문 편집기는 고급 모드에서만 제공
- 서비스 생성/수정 wizard에서 등록된 도메인을 선택하거나 새 도메인을 즉시 추가할 수 있게 한다
- 선택한 도메인, 내부 port, SSL 방식으로 `service_domains` row와 nginx 설정을 자동 생성한다
- form에는 `이 도메인으로 접속하면 이 서비스의 이 포트로 연결됨` helper 문구를 표시한다
- 도메인에 업로드 인증서가 있으면 자동 연결
- 인증서가 없으면 서비스 화면에서 certbot 무료 인증서 발급 버튼 제공
...
- Harbor Compose 템플릿을 Docker Infra 내부 리소스로 포함
- 최초 구성 마법사에서 활성화 시 Harbor Compose 실행
- 시스템 설정에서 백업 시스템 시작/정지/재시작 제공
- Harbor 관리자 계정과 내부 token을 사용자가 직접 다루지 않게 숨김
- Harbor URL은 내부적으로 고정하거나 Docker Infra nginx 경유로 노출
- Harbor data directory 용량, 사용량, 남은 용량 계산
- 백업 시스템 disabled 상태에서도 서비스 배포는 정상 동작

### 6.2 서비스 이미지 백업

필요한 TODO:

- 서비스 배포 시 사용 이미지와 digest를 기록
- 백업 시스템 활성화 시 서비스 이미지 tag를 내부 Harbor에 저장
- 백업 tag 규칙 정의: `{service_namespace}/{image_name}:{version_or_timestamp}`
- 동일 digest 중복 백업 방지
- 백업 성공/실패를 서비스 상세에 표시
- 특정 서비스의 백업 버전 목록과 복원 버튼 제공
- 복원 시 Compose 이미지 tag를 백업 tag로 바꾸고 재배포

### 6.3 백업 저장소 정리

필요한 TODO:

- 시스템 설정에 미사용 N일 이상 이미지 일괄 삭제 기능
- 현재 서비스에서 사용하지 않는 이미지 일괄 삭제 기능
- 서비스별 보존 개수 정책
- 백업 시스템 정지 기능
...
- 외부 Harbor 프로젝트/저장소 관리 UI 제거 또는 백업 시스템 내부 관리로 축소
- 서버별 로컬 이미지 목록 유지
- 사용 중/미사용 이미지 필터 유지
...
- backup Harbor가 비활성화된 경우 이미지 화면은 로컬 이미지만 보여줌
...
- 인증서 파일 permission과 private key 보호 정책 강화
- chain/fullchain 업로드 지원 여부 결정
- 인증서가 어떤 서비스에 적용 중인지 표시
...
- 외부 Harbor integration table 제거 또는 backup system config로 대체
- [Done] backup_system_settings 테이블 설계
- service_image_backups 테이블 추가
