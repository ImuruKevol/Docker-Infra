...
| 파일 | 현재 역할 | 영향 |
|---|---|---|
| `src/model/struct/service_volume_backups.py` | named volume을 tar.gz로 묶어 ORAS로 Harbor에 push/pull | 신규 기본 경로에서는 사용 중단, legacy restore 전용으로 축소 |
| `src/model/struct/service_image_backups.py` | Compose image 이력 기록 | 유지 |
| `src/model/struct/service_image_backup_scheduler.py` | 컨테이너 스냅샷과 named volume 백업 자동 실행 | volume 백업 부분을 CephFS snapshot 생성으로 교체 |
...
| `src/model/struct/services_rollback.py` | Compose/image/volume rollback | ORAS volume restore 대신 CephFS snapshot restore로 전환 |
...
| Legacy named volume | 고급 옵션 | 기존 Docker named volume 방식 |
...
다른 서버로 서비스를 옮길 때는 데이터 migration이 필요합니다.
...
저장소
  ...
  - named volume legacy 선택 시 경고가 필요한지
```
...
현재 롤백 modal은 image restore와 named volume restore를 같이 보여준다.
...
named volume 문구는 legacy 서비스에서만 보여준다.
...
storage.migration.named_volume_to_cephfs
```
...
| mount_name | data |
| backend | cephfs, local_bind, named_volume_legacy |
| original_source | 원래 named volume 이름 |
...
src/model/struct/storage_migration.py
...
| `storage_migration.py` | named volume → CephFS migration |
...
| `migration_plan` | named volume migration 계획 |
| `migration_run` | migration 실행 |
...
4. top-level named volume은 제거한다.
...
- named volume을 보면 CephFS 변환 후보로 표시한다.
...
현재는 `service_volume_backups.version_restore_context()`를 통해 ORAS artifact를 본다.

변경:

- CephFS 서비스는 `storage_snapshots.rollback_context()`를 본다.
- legacy named volume 서비스만 기존 ORAS context를 본다.
...
```text
이 서비스는 legacy named volume을 사용합니다. ORAS volume artifact 복원이 필요합니다.
```

## 17. 기존 ORAS named volume 기능 처리

ORAS volume 백업 코드는 바로 삭제하지 않는다.

대신 단계적으로 축소한다.

### 17.1 유지할 기능

- 이미 생성된 `service_volume_backups` 이력 조회
- legacy named volume artifact restore
- 기존 고객 데이터 보호를 위한 read-only 성격의 호환

### 17.2 기본 경로에서 제거할 기능

- 신규 서비스 자동 named volume 백업
- 자동 백업 정책의 volume_to_harbor 실행
- 서버 등록 시 ORAS 필수 설치 안내
...
named volume tar.gz → ORAS → Harbor
```
...
## 18. named volume migration wizard

기존 서비스를 CephFS로 옮기는 화면이 필요하다.

위치:

```text
/services/{service_id}/storage
```

또는 서비스 상세 저장소 탭 안의 버튼:
...
```text
1. Compose에서 named volume 찾기
2. 실행 중 container와 node 찾기
3. CephFS target path 생성
4. 서비스 중지 가능 여부 확인
5. named volume 내용을 임시 컨테이너로 tar export
6. CephFS path에 extract
7. Compose를 bind mount로 변경
8. 서비스 재배포
9. 읽기/쓰기 검증
10. 기존 named volume은 보존 상태로 표시
```

사용자에게 보여줄 문구:

```text
기존 volume은 바로 지우지 않습니다.
복사와 재배포가 성공하면 보존 상태로 남겨두고, 사용자가 나중에 삭제할 수 있게 합니다.
```
...
초기에는 별도 table 없이 `operation_logs.result_payload`와 `services.metadata.storage_migration`에 남길 수 있다. 운영 기능으로 커지면 table을 분리한다.

## 19. AI Agent와 템플릿 영향

### 19.1 AI 서비스 생성 계약

AI가 stateful service를 만들 때 다음 metadata를 포함해야 한다.
...
AI가 named volume만 생성하면 Docker Infra가 CephFS bind mount로 바꿀 수 있어야 한다.

AI에게 줄 쉬운 규칙:

```text
업로드 파일, DB 파일, 사용자 데이터는 named volume 대신 storage mount로 표시한다.
```
...
템플릿 렌더 시 `${DOCKER_INFRA_STORAGE_DATA}`를 실제 CephFS path로 치환한다.
...
- 신규 서비스가 named volume 없이 CephFS host path를 bind mount로 사용한다.
...
### Phase 6: named volume migration

목표:

- 기존 named volume 서비스 CephFS로 이전

작업:

- `storage_migration.py`
- 서비스 상세 migration wizard
- Compose rewrite와 재배포

완료 기준:

- 기존 named volume 데이터가 CephFS path로 복사되고 서비스가 정상 실행된다.
- 기존 named volume은 자동 삭제하지 않는다.

### Phase 7: 자동 정책 정리

목표:

- 기존 자동 백업 정책에서 volume ORAS 경로 제거
- CephFS snapshot retention 적용

작업:

- `service_image_backup_scheduler.py` 조정
- `service_image_backup_cleanup.py`에서 legacy volume artifact만 정리
...
3. 클러스터 서비스는 named volume 대신 CephFS bind mount를 쓴다.
