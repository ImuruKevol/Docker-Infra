...
Harbor는 image 관리 때문에 계속 필수로 둔다. 하지만 named volume을 Harbor에 저장하려면 ORAS로 volume을 tar.gz 한 덩어리로 올려야 하고, 이 방식은 Docker image layer처럼 효율적인 변경분 저장이 되지 않는다.

따라서 장기 방향은 named volume 백업을 더 개선하는 것이 아니라, 서비스 저장소 자체를 bind mount 기반으로 바꾸는 것이다. bind mount의 실제 저장소는 Dockerized Ceph이 제공하는 CephFS에 둔다. Ceph은 Docker container로 실행하되, 각 노드마다 64GB 또는 128GB 같은 고정 크기 OSD 슬롯을 Docker Infra가 만들고 관리한다.

초등학생에게 설명하듯 말하면 다음과 같다.

- Harbor는 컨테이너 이미지 보관함이다.
- ORAS volume 백업은 가방 전체를 매번 택배로 보내는 방식이라 아깝다.
- Ceph은 여러 미니PC의 작은 저장 공간을 하나의 튼튼한 공용 책장처럼 묶는 역할이다.
- Docker Infra는 각 미니PC에 "64GB짜리 책장 칸"을 만들고 Ceph에게 맡긴다.
- 서비스 컨테이너는 named volume 대신 그 공용 책장의 정해진 칸을 bind mount로 쓴다.
...
2. named volume을 Harbor에 넣으려면 ORAS 같은 OCI artifact push 경로가 필요하다.
3. 현재 ORAS volume 백업은 volume 전체를 tar.gz 하나로 묶는다.
4. 파일 일부만 바뀌어도 tar.gz 전체 digest가 바뀐다.
5. 그래서 Docker image layer처럼 효율적으로 변경분만 저장하는 효과가 없다.
...
### 3.1 더 이상 기본값을 named volume으로 보지 않는다
...
| 모드 | 조건 | 저장소 기본값 | 쉬운 설명 |
|---|---|---|---|
| 독립 서버 | Swarm cluster에 아직 등록하지 않은 서버 | local bind mount 또는 legacy named volume | 혼자 쓰는 책상이다. 책상이 단순하고 빠르지만, 다른 집으로 자동 복사되지는 않는다. |
...
### 10.3 named volume에서 bind mount로 전환

신규 서비스는 기본적으로 bind mount를 사용한다.

기존 named volume 서비스는 migration 기능으로 옮긴다.

```text
1. 서비스 중지
2. named volume 내용을 임시 컨테이너로 tar export
3. CephFS target path 생성
4. tar extract
5. Compose volume을 bind mount로 변경
6. 서비스 재배포
7. 검증 성공 후 named volume 보존 기간 뒤 삭제
```
...
ORAS 방식에서는 volume 전체 tar.gz가 버전이었다.
...
- named volume 대신 CephFS bind mount
...
Harbor는 계속 image 전용으로 유지한다. volume 백업을 다시 Harbor/ORAS로 밀어 넣지 않는다.
...
2. named volume은 legacy 또는 local-only 옵션으로만 둔다.
...
- ORAS volume backup은 장기 방향에서 제외한다.
...
- 기존 named volume은 migration wizard 제공
...
- 독립 서버에서 만든 local bind mount 서비스는 다른 서버로 이동하기 전에 migration plan을 요구한다.
- Swarm 등록 후에는 local bind mount를 CephFS로 옮기는 migration wizard를 제공한다.
...
### 18.3 named volume migration 검증

- 기존 named volume service 생성
- 데이터 작성
- CephFS path로 migration
- Compose bind mount 전환
- 서비스 정상 실행 확인
...
목표는 Docker image layer와 완전히 같은 숫자를 만드는 것이 아니다. 목표는 "전체 tar.gz를 매번 저장하는 ORAS 방식보다 훨씬 덜 증가하는지"를 확인하는 것이다.
...
2. ORAS volume artifact는 장기 방향에서 제외한다.
3. 클러스터 서비스의 named volume 기본값을 CephFS bind mount로 전환한다.
