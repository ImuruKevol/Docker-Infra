...
P3-03부터 `/system` 화면에서 일반 설정과 연동 enabled 상태를 저장한다.

| 설정 key | 설명 |
|---|---|
| `general.browser_title` | 브라우저 title 기본값 |
| `general.favicon_url` | favicon URL |
| `general.logo_url` | sidebar/logo image URL |
| `integration.harbor.enabled` | Harbor 연동 사용 여부 |
| `integration.gitlab.enabled` | GitLab 연동 사용 여부 |
| `integration.cloudflare.enabled` | Cloudflare 연동 사용 여부 |

연동 secret은 `integration.{name}.{secret}` key에 `is_secret=true`로 저장한다. 조회 응답은 `value=null`과 `secret.masked_value="********"`만 반환하며 원문 secret은 API/UI에 노출하지 않는다.

sidebar menu는 `/api/system/settings`의 enabled 값을 읽어 연동 메뉴를 필터링한다. 현재 UI에서는 Cloudflare disabled 시 도메인 메뉴, Harbor disabled 시 이미지 메뉴를 숨긴다. GitLab은 이후 빌드 버튼이 생기는 화면에서 button visibility에 사용한다.
