...
def default_templates():
    return [
        ...
        },
        {
            "name": "GitLab CE",
            "namespace": "gitlab_ce",
            "description": "공식 Docker Compose 문서를 기준으로 한 GitLab Community Edition 템플릿",
            "metadata": {"category": "service", "primary_image": "gitlab/gitlab-ce:18.11.1-ce.0"},
            "files": {
                "docker-compose.yaml": textwrap.dedent(
                    """
                    services:
                      gitlab:
                        ...
                        volumes:
                          ...
                          - {{ logs_volume }}:/var/log/gitlab
                          - {{ data_volume }}:/var/opt/gitlab
                        healthcheck:
                          test: ["CMD-SHELL", "curl -fkLs http://127.0.0.1/-/health || exit 1"]
                          interval: 60s
                          timeout: 10s
                          retries: 20
                    """
                ...
                "values.default.yaml": textwrap.dedent(
                    ...
                    https_port: 8443
                    ssh_port: 2222
                    config_volume: gitlab_config
                    logs_volume: gitlab_logs
                    data_volume: gitlab_data
                    """
                ).strip() + "\n",
                "values.schema.json": _schema("GitLab CE", "GitLab Community Edition Docker Compose 템플릿", [
                    {"name": "namespace", "title": "서비스 ID", "default": "gitlab_ce"},
                    {"name": "service_name", "title": "Compose 서비스 이름", "default": "gitlab"},
                    {"name": "image", "title": "이미지", "default": "gitlab/gitlab-ce:18.11.1-ce.0"},
                    {"name": "external_url", "title": "외부 URL", "default": "http://gitlab.local"},
                    {"name": "http_port", "title": "HTTP 포트", "type": "integer", "default": 8080},
                    {"name": "https_port", "title": "HTTPS 포트", "type": "integer", "default": 8443},
                    {"name": "ssh_port", "title": "SSH 포트", "type": "integer", "default": 2222},
                    {"name": "config_volume", "title": "설정 Volume", "default": "gitlab_config"},
                    {"name": "logs_volume", "title": "로그 Volume", "default": "gitlab_logs"},
                    {"name": "data_volume", "title": "데이터 Volume", "default": "gitlab_data"},
                ], ["namespace", "service_name", "image", "external_url", "http_port", "https_port", "ssh_port", "config_volume", "logs_volume", "data_volume"]),
                "README.md": "\n".join([
                    "# GitLab CE",
                    "",
                    "GitLab 공식 Docker Compose 설치 문서를 기준으로 한 Community Edition 템플릿입니다.",
                    "",
                    "- 기본 이미지: `gitlab/gitlab-ce:18.11.1-ce.0`",
                    "- 기본 포트: 8080, 8443, 2222",
                    "",
                    "운영 환경에서는 `external_url`과 포트를 실제 접속 정보에 맞게 조정해야 합니다.",
                    "",
                ]),
            },
        },
        {
            ...
            "files": {
                ...
                "values.schema.json": _schema("Harbor Registry", "Harbor 레지스트리 스택 초안 템플릿", [
