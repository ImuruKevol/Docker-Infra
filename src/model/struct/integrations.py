...
SPECS = {
    ...
    "gitlab": {
        "label": "GitLab",
        "table": "integration_gitlab",
        "fields": ["url"],
        "secret_name": "token",
        "secret_column": "token_enc",
    },
}
...
class Integrations:
    ...
    def test_connection(self, key, payload, env=None):
        ...
        try:
            ...
            if key == "gitlab":
                return self._test_gitlab(fields, secret_value)
    ...
    def _test_gitlab(self, fields, secret_value):
        url = _normalize_url(fields.get("url"))
        if url == "":
            raise IntegrationError(400, "GitLab URL을 입력해주세요.", "GITLAB_URL_REQUIRED")
        if str(secret_value or "").strip() == "":
            raise IntegrationError(400, "GitLab token을 입력해주세요.", "GITLAB_TOKEN_REQUIRED")

        endpoint = f"{url}/api/v4/user"
        headers = {"PRIVATE-TOKEN": secret_value, "Accept": "application/json"}
        try:
            status, payload = _request_json(endpoint, headers=headers)
        except urlerror.HTTPError as exc:
            message = exc.read().decode("utf-8", errors="ignore")
            raise IntegrationError(exc.code, message or "GitLab 연결에 실패했습니다.", "GITLAB_CONNECTION_FAILED")
        except urlerror.URLError as exc:
            raise IntegrationError(502, str(exc.reason), "GITLAB_CONNECTION_FAILED")
        if status != 200:
            raise IntegrationError(status, "GitLab 연결에 실패했습니다.", "GITLAB_CONNECTION_FAILED")
        return {
            "message": "GitLab 연결에 성공했습니다.",
            "summary": {
                "endpoint": endpoint,
