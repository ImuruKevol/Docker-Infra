...
class DomainManagementUiStaticContractTest(unittest.TestCase):
    ...
    def test_ddns_management_server_contract_is_wired(self):
        ...
        self.assertIn("table(class=\"w-full min-w-[1100px]", template)
        self.assertIn("th(class=\"w-24 whitespace-nowrap px-4 py-2 text-left font-semibold\") 상태", template)
        self.assertIn("th(class=\"w-36 whitespace-nowrap px-4 py-2 text-left font-semibold\") Dispatcher 요청", template)
        self.assertIn("th(class=\"w-28 whitespace-nowrap px-4 py-2 text-left font-semibold\") IP", template)
        self.assertIn("연결 서비스", template)
        self.assertIn("연결 대상", template)
        self.assertIn("th(class=\"w-24 px-4 py-2 text-center font-semibold\") 등록", template)
        ...
        self.assertIn("span(class=\"min-w-0 break-all\") {{item.domain}}", template)
        self.assertIn("fa-arrow-up-right-from-square ml-2 mr-2", template)
