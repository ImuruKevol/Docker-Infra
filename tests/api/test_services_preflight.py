...
""")
    ...
    def test_service_create_supports_templates_and_draft_sources(self):
        ...
        self.assertIn("def storage_preview():", api)
        self.assertIn("wizard.storage_preview", api)
        ...
        self.assertIn("public async refreshStoragePreview", view)
        self.assertIn("public storagePayload", view)
        self.assertIn("storage_preview", view)
        self.assertIn("await this.refreshStoragePreview({ silent: true })", view)
        ...
        for token in ["템플릿 선택", "기본 정보", "템플릿 변수", "저장소 확인", "도메인 설정", "생성 요약", "DDNS 사용"]:
            self.assertIn(token, template)
        for token in ["storageMounts()", "storageFallback()", "confirmLocalStorageFallback()", "goStorageSettings()", "DB snapshot hook 권장"]:
            self.assertIn(token, template)
        ...
        self.assertIn("def storage_preview", wizard)
        self.assertIn('"source": "service_create_storage_step"', wizard)
        ...
        self.assertIn("versionBackupEntries(version)", services_template)
        self.assertIn("versionBackupEntryTitle(backup)", services_template)
    ...
    def test_service_rollback_contract_is_wired(self):
        ...
        self.assertIn("SERVICE_ROLLBACK_BACKUP_EXPIRED", rollback)
        self.assertIn("expired_count", rollback)
        self.assertIn("container_snapshot_target", rollback)
