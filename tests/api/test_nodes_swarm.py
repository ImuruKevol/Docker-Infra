...
class NodesSwarmStaticContractTest(unittest.TestCase):
    def test_node_routes_and_openapi_contract_are_declared(self):
        ...
        self.assertIn('data-testid="servers-node-list"', servers_view)
        self.assertIn('(click)="openAddServer()"', servers_view)
        self.assertNotIn('(click)="ensureLocalMaster()"', servers_view)
        self.assertNotIn("(click)=\"load(selected()?.id || '')\"", servers_view)
        self.assertIn("runningServiceGroups", servers_view_ts)
        self.assertIn("isLikelyPrivateHost", servers_view_ts)
        self.assertIn("실행 컨테이너", servers_view)
        self.assertIn("nodeServiceCount(node)", servers_view)
        self.assertIn("nodeRunningContainerCount(node)", servers_view)
        self.assertIn("nodeServiceCount", servers_view_ts)
        self.assertIn("nodeRunningContainerCount", servers_view_ts)
        self.assertNotIn("roleDescription", servers_view)
        self.assertNotIn("roleDescription", servers_view_ts)
        self.assertNotIn("이 서비스가 실행 중인 서버", servers_view_ts)
        self.assertNotIn("저장된 SSH key로 관리하는 서버", servers_view_ts)
        self.assertNotIn("사설 ${host}", servers_view_ts)
        self.assertIn("return publicIp;", servers_view_ts)
        self.assertNotIn("return publicIp ? `사설 ${privateHost} · 공인 ${publicIp}`", servers_view_ts)
        self.assertIn("NodeDeleteMixin", nodes_model)
        self.assertIn("list_with_runtime_summary", nodes_model)
